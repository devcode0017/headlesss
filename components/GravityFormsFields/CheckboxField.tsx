...
export default function CheckboxField({ field, fieldErrors }: Props) {
  ...
  function handleChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const otherCheckboxValues = checkboxValues.filter((checkboxValue: CheckboxInput) => checkboxValue.inputId !== Number(name));
  ...
  return (
    <fieldset id={htmlId} className={`gfield gfield-${type} ${cssClass}`.trim()}>
      ...
      {checkboxInputs.map(({ id: inputId, text, value }) => (
        <div key={inputId}>
          <input type='checkbox' name={String(inputId)} id={`input_${formId}_${id}_${inputId}`} value={String(value)} onChange={handleChange} />
